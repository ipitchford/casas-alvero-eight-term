...
from native_options import add_native_options, native_flags
...
def main():
    ...
    parser.add_argument('--rerun-native',action='store_true',help='Repeat the full native enumeration in temporary files (default native guard: 240 seconds); otherwise check saved receipts only.')
    add_native_options(parser, native_default=240)
    ...
    if args.rerun_native:
        result['freshNativeReplay'] = run_fresh_native(args, combined_output)
    print(json.dumps(result,indent=2))


def run_fresh_native(args, expected_output):
    # Native output lives only in a new scratch directory; frozen receipts stay read-only.
    with tempfile.TemporaryDirectory(prefix='casas-row9-m6-review-') as temporary:
        scratch=Path(temporary)
        command=[sys.executable, '-B', str(HERE/'run_native.py'), '--role', 'replay',
                 '--output', str(scratch)]+native_flags(args)
        # The worker owns the compiler/native deadlines. Do not kill it with a
        # competing subprocess timeout: that could orphan the guarded native child.
        # replay.py supplies the outer checker deadline and kills its process group.
        ...
        require((scratch/'replay.log').read_bytes()==expected_output,'fresh native replay differs')
        return {'status':'PASS','elapsedSeconds':fresh['elapsedSeconds'],
                'outputSHA256':fresh['outputSHA256'], 'compilerArgv':fresh['compilerArgv'],
                'nativeWallLimitSeconds':args.native_timeout,
                'compileWallLimitSeconds':args.compile_timeout,
