"""Replay complete native enumeration and all precision certificates.
...
def main():
    ...
    add_native_options(parser, native_default=90)
    ...
    presentation_raw=(HERE.parent/'elimination/presentation.json').read_bytes()
    ...
    require(active_sizes in ([3,4],[5]),'unauthorized batch range')
    ...
    # Execute the entire independent native enumeration, under an external guard.
    native_start=time.monotonic()
    compiler = compiler_argv(args.cxx)
    subprocess.run(compiler+['-std=c++17','-O2',str(HERE/'replay_residues.cpp'),'-o',str(HERE/'replay_residues')],
                   capture_output=True,text=True,timeout=args.compile_timeout,check=True)
    native=subprocess.run([str(HERE/'replay_residues'),str(destination/'native-input.txt')],
                          capture_output=True,text=True,timeout=args.native_timeout,check=True)
    lines=iter(native.stdout.splitlines())
    ...
    total_orbits=0; counts={}; summaries=[]
    ...
    for case,cert in zip(residue['cases'],lifts['cases']):
        ...
        for record in cert['orbits']:
            ...
            require(record['status']=='EXCLUDED' and any(td),'missing obstruction')
            valuation=0
        ...
        require(cert['status']=='EXCLUDED','case status')
    ...
    result={'status':'PASS','scope':'Specified complete canonical systems, including all extension roots and coefficient degenerations.',
